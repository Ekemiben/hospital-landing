...
const MedicalEquipment = ({ equipment = [] }) => {
  const diagnosticEquipment = equipment.filter(
    (item) => item?.category === "Diagnostic"
  );

  const treatmentEquipment = equipment.filter(
    ...
    <section id="equipment" className="py-16 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {diagnosticEquipment.map((item, index) => {
              const Icon = item?.icon || FaHospitalSymbol;

              return (
                <div
                  key={index}
                  className="bg-gray-50 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 text-center flex flex-col items-center"
                >
                  {Icon && (
          ...
          <div className="mt-8 text-center">
            <img
              src="/placeholder-diagnostic-room.jpg"
              alt="Advanced Diagnostic Room"
              className="rounded-lg shadow-xl mx-auto w-full max-w-3xl"
            />
            <p className="mt-4 text-gray-600 italic">
              Our cutting-edge diagnostic tools ensure accurate and timely
              results.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {treatmentEquipment.map((item, index) => {
              ...
              return (
                ...
                >
                  {Icon && (
                    <Icon className="text-5xl text-orange-600 mb-4" />
                  )}
                  <h4 className="text-xl font-semibold text-gray-800">
                    {item?.name}
                  </h4>
          ...
          <div className="mt-8 text-center">
            <img
              src="/placeholder-operating-room.jpg"
              alt="State-of-the-art Operating Theatre"
              className="rounded-lg shadow-xl mx-auto w-full max-w-3xl"
            />
            <p className="mt-4 text-gray-600 italic">
              Equipped with international standard operating theatres for
              complex surgeries.
            </p>
          </div>
        </div>
